      ...
      {!explanation ? (
        <div className="rounded-[16px] bg-[var(--color-surface-sunken)] p-4 sm:p-5">
          <div className="flex items-start gap-3">
            ...
            <div>
              <h3 className="text-[15px] font-semibold text-[var(--color-text-primary)]">Generate a readable summary</h3>
              <p className="mt-1 text-[15px] leading-relaxed text-[var(--color-text-secondary)]">Transparent rules calculate the facts first. AI only explains the de-identified trends and limitations.</p>
            </div>
          </div>

          <div className="mt-4 rounded-[14px] bg-[var(--color-surface)] p-3.5 text-[15px] leading-relaxed text-[var(--color-text-secondary)]">
        ...
        <div className="space-y-4">
          <Card className="p-5">
            <div className="flex flex-wrap items-start justify-between gap-3">
              <div>
                <p className="text-[14.5px] font-medium uppercase tracking-wide text-[var(--color-text-tertiary)]">AI-generated explanation</p>
                <h3 className="mt-1 text-[18px] font-semibold text-[var(--color-text-primary)]">{explanation.headline}</h3>
              </div>
              <div className="text-right text-[14.5px] text-[var(--color-text-tertiary)]">
                <p>{explanation.model ?? "Evidence-based local engine"}</p>
                <p>{new Date(explanation.generatedAt).toLocaleString()}</p>
              </div>
            </div>
            <p className="mt-3 text-[14px] leading-relaxed text-[var(--color-text-secondary)]">{explanation.overview}</p>
          </Card>
          ...
          <Card className="p-4">
            <h4 className="text-[14px] font-semibold text-[var(--color-text-primary)]">Uncertainty and data limits</h4>
            <p className="mt-1 text-[15px] leading-relaxed text-[var(--color-text-secondary)]">{explanation.uncertainty}</p>
          </Card>

          {payload.scientificBasis.length > 0 && (
            <Card className="p-4">
              <h4 className="text-[14px] font-semibold text-[var(--color-text-primary)]">Scientific grounding supplied to the AI</h4>
              <p className="mt-1 text-[14.5px] leading-relaxed text-[var(--color-text-tertiary)]">The model receives these source titles and controlled evidence notes; it does not browse the web or invent new citations.</p>
              <div className="mt-3 flex flex-wrap gap-2">
                {payload.scientificBasis.map((source) => (
                  <a key={source.id} href={source.url} target="_blank" rel="noreferrer" className="inline-flex items-center gap-1 rounded-full border border-[var(--color-border)] px-2.5 py-1.5 text-[14.5px] font-medium text-[var(--color-accent)] hover:bg-[var(--color-accent-soft)]">
...
function ExplanationList({ title, items }: { title: string; items: string[] }) {
  return (
    <Card className="p-4">
      <h4 className="text-[14.5px] font-semibold text-[var(--color-text-primary)]">{title}</h4>
      <ul className="mt-3 space-y-2.5">
        {items.map((item) => (
          <li key={item} className="flex gap-2 text-[15px] leading-relaxed text-[var(--color-text-secondary)]">
