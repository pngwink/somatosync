    ...
    <Card className="p-6 sm:p-7">
          ...
          <div className="mt-5 divide-y divide-[var(--color-border)] rounded-[16px] bg-[var(--color-surface-sunken)] px-4 sm:px-5">
            {patterns.slice(0, 3).map((pattern) => (
              <div key={pattern.id} className="flex items-start gap-3 py-4">
                ...
                <div><p className="text-[16px] font-semibold text-[var(--color-text-primary)]">{pattern.title}</p><p className="mt-1 text-[16px] leading-6 text-[var(--color-text-secondary)]">{pattern.detail}</p></div>
              </div>
            ))}
          </div>
        ) : (
      ...
      <section aria-labelledby="recovery-story-heading" className="mt-7 border-t border-[var(--color-border)] pt-7">
        ...
        {story.length > 0 ? (
          ...
          <div className="mt-5 rounded-[16px] bg-[var(--color-surface-sunken)] p-5"><p className="text-[16px] font-semibold text-[var(--color-text-primary)]">Your recovery story will build automatically</p><p className="mt-1 text-[16px] leading-6 text-[var(--color-text-secondary)]">Check-ins, assessments, adaptive reading, shared-support feedback, and context you choose to add will appear here.</p></div>
        )}
      </section>
    </Card>
  );
