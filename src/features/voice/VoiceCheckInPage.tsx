  ...
  return (
    <div className="flex items-center gap-2" aria-label={`Severity ${value} out of 6`}>
      ...
      <div className="min-w-[96px] rounded-[12px] bg-[var(--color-surface-sunken)] px-3 py-2 text-center">
        <p className="text-[16px] font-semibold tabular-nums">{value} / 6</p>
        <p className="text-[16px] text-[var(--color-text-secondary)]">{severityLabel(value)}</p>
      </div>
    ...
    return (
      <div className="mx-auto max-w-3xl py-8">
        <Card className="p-7 text-center">
          ...
          <p className="mt-2 text-[16px] text-[var(--color-text-secondary)]">Confirmed PCSS severity: {savedTotal} / 132. The recording and transcript were discarded.</p>
    ...
    <div className="mx-auto w-full max-w-5xl space-y-5">
        ...
        <Card className="p-6 sm:p-8">
          <div className="flex gap-3">
            ...
            <div className="max-w-2xl">
              ...
              <p className="mt-2 text-[16px] leading-relaxed text-[var(--color-text-secondary)]">Talk normally about what you noticed, what you were doing, and what made things easier or harder. SomatoSync will turn that into a draft check-in for you to review.</p>
            </div>
          </div>
          {modelBusy && progress?.percent != null && (
            <p className="mt-4 text-[16px] font-medium text-[var(--color-accent)]">Getting ready · {progress.percent}%</p>
          )}
      ...
      {step === "record" && (
        <Card className="p-7 text-center sm:p-9">
          ...
          <p className="mx-auto mt-2 max-w-xl text-[16px] leading-relaxed text-[var(--color-text-secondary)]">{processing ? progress?.label ?? "Understanding your check-in" : `Talk naturally for up to ${MAX_RECORD_SECONDS} seconds · ${recordSeconds}s`}</p>
        ...
        <div className="space-y-4">
            ...
            <Card className="border-[var(--color-risk)]/40 bg-[var(--color-risk-soft)] p-5">
              <div className="flex items-start gap-3">
                ...
                <div>
                  ...
                  <p className="mt-1 text-[16px] leading-relaxed text-[var(--color-text-secondary)]">If any listed sign is actually happening, seek urgent medical evaluation rather than relying on this app. Correct the transcript only when the recognition was wrong.</p>
                  <div className="mt-3 flex flex-wrap gap-2">{extraction.dangerSigns.map((sign) => <Badge key={sign} tone="risk">{sign}</Badge>)}</div>
                  <a href="https://www.cdc.gov/heads-up/signs-symptoms/index.html" target="_blank" rel="noreferrer" className="mt-3 inline-flex text-[16px] font-semibold text-[var(--color-risk)] underline">Open CDC danger signs</a>
                </div>
              </div>
            </Card>
          )}
          ...
          <Card className="p-5 sm:p-7">
            <div className="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
              <div>
                ...
                <p className="mt-1 max-w-2xl text-[16px] leading-relaxed text-[var(--color-text-secondary)]">SomatoSync turned what you said into tentative symptom ratings and context. Change any number that does not match how you actually feel before saving.</p>
              </div>
              <div className="rounded-[14px] bg-[var(--color-surface-sunken)] px-4 py-3 text-left sm:text-right">
                <p className="text-[16px] text-[var(--color-text-tertiary)]">Current total</p>
            ...
            {extraction.symptoms.length > 0 ? (
                ...
                {extraction.symptoms.map((symptom) => (
                  <div key={symptom.id} className="rounded-[var(--radius-md)] border border-[var(--color-border)] bg-[var(--color-surface-raised)] p-4">
                    <div className="flex items-start justify-between gap-3">
                      <div>
                        <p className="text-[16px] font-semibold">{symptom.label}</p>
                    ...
                    <div className="mt-4 rounded-[12px] bg-[var(--color-surface-sunken)] p-3">
                      <p className="text-[16px] font-semibold uppercase tracking-wide text-[var(--color-text-tertiary)]">Why this score</p>
                      <p className="mt-1 text-[16px] leading-relaxed text-[var(--color-text-secondary)]">{symptom.rationale}</p>
                      <p className="mt-2 text-[16px] italic leading-relaxed text-[var(--color-text-tertiary)]">“{symptom.evidence}”</p>
                    </div>
                  </div>
                ))}
              </div>
              ...
              <div className="mt-5 rounded-[var(--radius-md)] bg-[var(--color-caution-soft)] p-4">
                <p className="text-[16px] font-semibold">No symptom was confidently matched</p>
                <p className="mt-1 text-[16px] text-[var(--color-text-secondary)]">You can still review the four symptom groups below, edit the transcript, or record again.</p>
              </div>
            )}

            {extraction.activityContexts.length > 0 && (
              <div className="mt-5"><p className="text-[16px] font-semibold uppercase tracking-wide text-[var(--color-text-tertiary)]">Context understood</p><div className="mt-2 flex flex-wrap gap-2">{extraction.activityContexts.map((context) => <Badge key={context} tone="info">{context}</Badge>)}</div></div>
            )}
          </Card>
          ...
          <Card className="p-5 sm:p-7">
            <div className="flex items-start justify-between gap-3">
              <div>
                ...
                <p className="mt-1 text-[16px] leading-relaxed text-[var(--color-text-secondary)]">Unmentioned symptoms start at zero. Open only the groups that apply and change any symptom that is present.</p>
              </div>
                ...
                return (
                  <details key={category} className="group rounded-[var(--radius-md)] border border-[var(--color-border)] bg-[var(--color-surface-raised)]">
                    <summary className="flex cursor-pointer list-none items-center justify-between gap-3 px-4 py-3.5">
                      <div>
                        <p className="text-[16px] font-semibold">{pcssCategoryLabels[category]}</p>
                        <p className="mt-0.5 text-[16px] text-[var(--color-text-secondary)]">{items.length} not mentioned{activeCount > 0 ? ` · ${activeCount} added` : ""}</p>
                      </div>
                      <ChevronDown className="h-4 w-4 text-[var(--color-text-tertiary)] transition-transform group-open:rotate-180" />
                    </summary>
                    <div className="border-t border-[var(--color-border)] px-4 py-2">
                      {items.length === 0 ? <p className="py-3 text-[16px] text-[var(--color-text-secondary)]">All symptoms in this group were already identified from your speech.</p> : items.map((symptom) => (
                        <label key={symptom.id} className="flex items-center justify-between gap-3 border-b border-[var(--color-border)]/70 py-3 last:border-b-0">
                          <span className="text-[16px] font-medium">{symptom.label}</span>
                          <select value={ratings[symptom.id]} onChange={(event) => updateRating(symptom.id, Number(event.target.value))} className="min-w-[132px] rounded-[10px] border border-[var(--color-border)] bg-[var(--color-surface)] px-2.5 py-2 text-[16px]">
          ...
          <details className="rounded-[var(--radius-lg)] border border-[var(--color-border)] bg-[var(--color-surface)] p-4 shadow-[var(--shadow-low)]">
            <summary className="flex cursor-pointer list-none items-center justify-between gap-3">
              <div><p className="text-[16px] font-semibold">See or correct the transcript</p><p className="mt-0.5 text-[16px] text-[var(--color-text-secondary)]">The transcript is not stored after you save.</p></div>
              <Pencil className="h-4 w-4 text-[var(--color-accent)]" />
            </summary>
            <textarea value={transcript} onChange={(event) => setTranscript(event.target.value)} rows={4} className="mt-4 w-full resize-y rounded-[var(--radius-md)] border border-[var(--color-border)] bg-[var(--color-surface-sunken)] px-3 py-2.5 text-[16px] leading-relaxed text-[var(--color-text-primary)] outline-none focus:border-[var(--color-accent)]" aria-label="Editable voice transcript" />
      ...
      {error && <div role="alert" className="rounded-[var(--radius-md)] border border-[var(--color-risk)]/35 bg-[var(--color-risk-soft)] p-3 text-[16px] text-[var(--color-risk)]">{error}</div>}
