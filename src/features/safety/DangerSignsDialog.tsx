  ...
  return (
    <Dialog open={open} onOpenChange={reset}>
      <DialogTrigger asChild>
        <button
          type="button"
          className="flex items-center gap-1.5 rounded-[var(--radius-sm)] border border-[color-mix(in_srgb,var(--color-risk)_35%,var(--color-border))] px-2.5 py-1.5 text-[16px] font-semibold text-[var(--color-risk)] hover:bg-[color-mix(in_srgb,var(--color-risk)_8%,transparent)]"
      ...
      <DialogContent className="max-h-[90vh] max-w-2xl overflow-y-auto">
        <div className="flex items-center gap-2 text-[var(--color-risk)]"><TriangleAlert className="h-5 w-5" /><span className="text-[16px] font-semibold uppercase tracking-wide">Emergency safety check</span></div>
        ...
        <div className="mt-5 space-y-2">
          {dangerSigns.map((sign, index) => (
            <label key={sign} className={`flex cursor-pointer items-start gap-3 rounded-[var(--radius-sm)] border p-3 ${checked[index] ? "border-[var(--color-risk)] bg-[color-mix(in_srgb,var(--color-risk)_7%,transparent)]" : "border-[var(--color-border)]"}`}>
              ...
              <span className="text-[16px] leading-relaxed text-[var(--color-text-primary)]">{sign}</span>
            </label>
          ))}
        </div>
        ...
        {hasDangerSign ? (
          <div className="mt-5 rounded-[var(--radius-md)] border-2 border-[var(--color-risk)] bg-[color-mix(in_srgb,var(--color-risk)_8%,var(--color-surface))] p-4">
            <h3 className="text-[16px] font-semibold text-[var(--color-risk)]">Seek emergency medical care now</h3>
            <p className="mt-1 text-[16px] leading-relaxed text-[var(--color-text-primary)]">Call local emergency services (911 in the U.S.) or go to the nearest emergency department. Do not rely on SomatoSync to monitor these symptoms.</p>
          </div>
        ) : (
          <div className="mt-5 rounded-[var(--radius-md)] bg-[var(--color-surface-sunken)] p-4">
            <p className="text-[16px] leading-relaxed text-[var(--color-text-secondary)]">No listed danger sign is selected. Continue monitoring changes and contact a healthcare professional for worsening, concerning, or persistent symptoms.</p>
          </div>
        )}
