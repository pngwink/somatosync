      ...
      <table className="w-full min-w-[560px] border-collapse text-left">
        ...
        <thead>
          <tr className="border-b border-[var(--color-border)] bg-[var(--color-surface-sunken)]">
            <th scope="col" className="px-4 py-2.5 text-[14.5px] font-medium uppercase tracking-wide text-[var(--color-text-tertiary)]">
              Category
            </th>
            <th scope="col" className="px-4 py-2.5 text-[14.5px] font-medium uppercase tracking-wide text-[var(--color-text-tertiary)]">
              Storage key
            </th>
            <th scope="col" className="px-4 py-2.5 text-right text-[14.5px] font-medium uppercase tracking-wide text-[var(--color-text-tertiary)]">
              Size
            </th>
            <th scope="col" className="px-4 py-2.5 text-left text-[14.5px] font-medium uppercase tracking-wide text-[var(--color-text-tertiary)]">
              Format
            </th>
            <th scope="col" className="px-4 py-2.5 text-left text-[14.5px] font-medium uppercase tracking-wide text-[var(--color-text-tertiary)]">
              Last updated
          ...
          {entries.map((entry) => (
            <tr key={entry.key}>
              <th scope="row" className="px-4 py-3 text-[14.5px] font-medium text-[var(--color-text-primary)]">
                {entry.category}
                {entry.isDemoData && (
                  <span className="ml-2 text-[14.5px] font-normal text-[var(--color-text-tertiary)]">(demo data)</span>
                )}
              </th>
              <td className="px-4 py-3 font-mono text-[14.5px] text-[var(--color-text-secondary)]">{entry.key}</td>
