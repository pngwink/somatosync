        ...
        <Card className="p-7">
          <div className="flex flex-col items-center text-center">
            ...
            {savedResult?.interpretationLabel && (
              <div className="mt-3">
                ...
                <p className="mt-1 max-w-xl text-[16px] leading-relaxed text-[var(--color-text-secondary)]">
          ...
          <div className="mt-6 grid gap-3 sm:grid-cols-2">
            {categoryOrder.map((category) => (
              <div key={category} className="rounded-[var(--radius-md)] border border-[var(--color-border)] p-3.5">
                <p className="text-[16px] text-[var(--color-text-tertiary)]">{pcssCategoryLabels[category]}</p>
          ...
          {guidanceTitles.length > 0 && (
            <div className="mt-5 rounded-[var(--radius-md)] border border-[var(--color-border)] p-4">
              <p className="text-[16px] font-medium uppercase tracking-wide text-[var(--color-text-tertiary)]">Guidance triggered by this entry</p>
              <ul className="mt-2 space-y-1.5 text-[16px] text-[var(--color-text-secondary)]">
          ...
          {topSymptoms.length > 0 && (
            <div className="mt-5 rounded-[var(--radius-md)] bg-[var(--color-surface-sunken)] p-4">
              <p className="text-[16px] font-medium uppercase tracking-wide text-[var(--color-text-tertiary)]">Highest reported today</p>
              <div className="mt-2 space-y-1.5">
                {topSymptoms.map((symptom) => (
                  <div key={symptom.id} className="flex items-center justify-between text-[16px]">
    ...
    <div className="mx-auto max-w-4xl space-y-5">
      ...
      <Card className="border-[var(--color-accent-soft-border)] p-4">
        <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
          <div><p className="text-[16px] font-semibold text-[var(--color-text-primary)]">Prefer to speak instead of moving 22 sliders?</p><p className="mt-1 text-[16px] text-[var(--color-text-secondary)]">The private conversational check-in understands natural language, proposes explained symptom scores, and lets you confirm everything before saving.</p></div>
      ...
      <Card className="p-4">
        <div className="flex items-start gap-3">
          ...
          <div className="text-[16px] leading-relaxed text-[var(--color-text-secondary)]">
            <p>
      ...
      <div className="sticky bottom-3 rounded-[var(--radius-lg)] border border-[var(--color-border)] bg-[var(--color-surface)] p-3 shadow-[var(--shadow-med)] backdrop-blur">
        <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
          <div>
            <p className="text-[16px] text-[var(--color-text-tertiary)]">Current symptom severity</p>
            <p className="font-mono text-[17px] font-semibold tabular-nums text-[var(--color-text-primary)]">
              ...
              <span className="ml-2 font-sans text-[16px] font-normal text-[var(--color-text-secondary)]">{summary.symptomCount} symptoms</span>
            </p>
          </div>
