    ...
    <section className={cn("rounded-[20px] border border-[var(--color-border)] bg-[var(--color-surface)]", className)}>
      <div className="flex items-start justify-between gap-5 px-6 pb-3 pt-6 sm:px-7 sm:pt-7">
        <div className="min-w-0">
          <h2 className="text-[21px] font-bold tracking-tight text-[var(--color-text-primary)]">{title}</h2>
          {description && <p data-focus-secondary="true" className="mt-1.5 max-w-[68ch] text-[16px] leading-7 text-[var(--color-text-secondary)]">{description}</p>}
        </div>
