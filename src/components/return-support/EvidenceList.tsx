...
export function EvidenceList({ items, tone, title }: EvidenceListProps) {
  ...
  return (
    <div>
      <p className={cn("text-[14.5px] font-medium uppercase tracking-wide", tone === "supporting" ? "text-[var(--color-positive)]" : "text-[var(--color-caution)]")}>
        {title}
      </p>
      <ul className="mt-2 space-y-2">
        {items.map((item, i) => (
          <li key={i} className="flex items-start gap-2 text-[14.5px] leading-snug text-[var(--color-text-primary)]">
            <Icon
